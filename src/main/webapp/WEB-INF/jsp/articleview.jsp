...
#articleimpo>.text>.companylogo>img {
	width: 100%;
	height: 100%;
}
...
#listbtn{
	...
	<script type="text/javascript">
		...
		function give() {
			var giver = "${loginedUser.userid}";
			var receiver = "${article.userid}";

			if (giver == receiver) {
				alert("자신에게는 기부 할 수 없습니다.");
			} else {
				$("#giveform").submit();
			}
	...
	<div id="articleview">
		...
		<div id="articleimpo">
			<div class="upper">
				<h3>${article.title}</h3>
				<p>작성자 : ${article.writer} 등록일 : ${article.date} 조회수 :
					${article.hit}</p>
			</div>
			<div class="text">

				<p>${article.body}</p>
				<p>전화 번호 : ${article.callno}</p>

				<form id="giveform" action="http://localhost:8099/card/give"
					method="post">
					<input type="hidden" name="giver" value="${loginedUser.userid}">
					<input type="hidden" name="receiver" value="${article.userid}">

				</form>
				<button id="givebtn" onclick="give();">기증하기</button>
