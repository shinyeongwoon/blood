		...
		<div id="cards">
			...
			<div id="userimpo">
				...
				<div class="text">
					...
					<p class="cardcnt">대출 나간 수 : ${loanOutCnt}</p>
				</div>
				...
				<c:forEach var="card" items="${cards}" varStatus="status">
					<p>
						<a href="http://localhost:8099/card/cardview?cardid=${card.id}"
							class="cardno"> card No : ${card.cardno}</a> <a
							href="http://localhost:8099/card/cardview?cardid=${card.id}"
							class="cardtext"> 등록일 : ${card.cdate} <br> 혈액형 :
							${card.cbg} <br> 헌혈 종류: ${card.kind} <br>
						<c:choose>
								<c:when test="${card.source == 2}">
									<span>대출카드</span>
								</c:when>
								<c:when test="${card.source == 1}">
									<span>기증카드</span>
								</c:when>
								<c:otherwise>
									<span>본인 카드</span>
